...
public class PolyphonicPitchDetection extends JPanel implements ActionListener {
    ...
    public void actionPerformed(ActionEvent e) {
        if ("beginPitchDetection".equals(e.getActionCommand())) {
            ...
            f0Init = 41.2;                   // Set reference f0 candidate
        ...
        if ("endPitchDetection".equals(e.getActionCommand())) {
            System.out.println("Vector stored notes:\n\n");
            for (BTNode i : detectedF0s) {
                if (i.data != (int) f0Init) {
                    int[] notePositions = new int[6];
                    notePositions = i.positions;
                    for (int j = 0; j < notePositions.length; j++) {
                        if (notePositions[j] != -1) {
                            System.out.println("String: " + j + " Fret: " + notePositions[j]);
                        }
                    }
                    System.out.print("\n");
                } else {
